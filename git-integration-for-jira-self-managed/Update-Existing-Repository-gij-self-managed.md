...
### Update Existing Repository properties
...
| _**realRoot**_ | _String_. **Optional on existing servers**.<br><br>This is the local path to the repository on the server where your Jira service is running. This will point the Git Integration for Jira app to a clone of the repository hosted locally with Jira.<br><br>This field corresponds to the **Repository root** input box in the Advanced setup/Repository settings.<br><br><img src='/wp-content/uploads/bbb-info-20.png' valign=middle /> The `realRoot` field may refer to an existing repository on a new server. If "root" doesn't exist, this field must be blank.<br><br><img src='/wp-content/uploads/bbb-note-20.png' valign=middle /> If this field is specified, `absoluteRoot` must also be defined. |
| _**absoluteRoot**_ | _Boolean_. Optional.<br><br>This field corresponds to the **Cloned root location** input field in the Advanced setup/Repository settings.<br><br>If set to **true**, the repository is automatically managed by Git Integration for Jira app (stored in `$JiraHOME`).<br><br>If set to **false**, the repository is manually configured by the Jira administrator.<br><br><img src='/wp-content/uploads/bbb-note-20.png' valign=middle /> If this field is specified, _**realRoot**_ must also be defined. |
